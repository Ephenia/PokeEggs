...
function createGemEgg() {
    ...
    function findByType(type: string) {
        ...
        Object.entries(speciesData).filter(([key, value]) => {
            if (!value.evolves_from_species) pkmnArr = Object.assign({ [key]: pkmnData[+key] }, pkmnArr);
        });
        ...
        for (const index of res) {
            ...
            if (!species.egg_groups.includes('no-eggs')) {
                ...
                for (const type of poke.types) {
                    const trueTypes = poke.types.filter((t: string) => t !== 'None');
                    if (type !== 'None') {
    ...
    function eggCost(pokeID: number, types: Array<string>) {
...
function createCheckSetting(name: string, setting: any) {
    const frag = new DocumentFragment();
...
function createDebugTools() {
    ...
    selected.addEventListener('input', (event: any) => {
    ...
    function pokeSearch(input: string) {
        const res = document.getElementById('debug-search-poke-res')!;
        disposeElement(res);
        const search: Pokemon = Object.entries(pkmnData).filter(([key, value]) => {
