...
function renderDaycare() {
    if (isHidden(daycareCont)) return;
...
function checkBreeding(skipCheck = false) {
    ...
    if (!skipCheck && player.daycareHandler.breedState) return;
    if (slot === -1) {
        startBreeding();
            ...
            if (findEmptySlot(daycare.pokemon) === -1 && daycare.domParent !== null && comp !== 0) {
                ...
                if (tickRate === 0 || randInt(tickRate) === 1) {
                    if (randRange(1, 100) <= eggChance[comp]) {
                        ...
                        checkPassFail = false;
                        if (player.settings.debugNotifs.state) Notify('errorMsg', `Egg failed to generate (Rate: ${tickRate}).`);
                    ...
                    if (player.settings.debugNotifs.state) Notify('warnMsg', `Breed karma increased by ${toAdd}.`);
                    daycare.breedKarma += toAdd;
                }
                //For the breeding tick bar
                const eggProg: HTMLElement = document.querySelector('#daycare-cont-prog-bar > div')!;
                if (!isHidden(eggProg)) {
                ...
                const eggProgDebug: HTMLElement = document.getElementById('daycare-cont-prog-debug')!;
                if (!isHidden(eggProgDebug)) eggProgDebug.textContent = `Odds: ${fracToPerc(daycare.eggChance)}% (${daycare.eggChance})`
            } else {
                //stopBreeding();
            }
        }, 1000);
...
function createDaycareEgg(parent: any, chainID: number) {
    const daycare = player.daycareHandler;
